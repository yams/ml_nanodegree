...
# Shuffled already in above methods, first 10,000 will go to val_X and val_y. Rest will be train_X and train_y.
...
pd.concat(train_y, train_X, axis=1).to_csv(os.path.join(data_dir, 'train.csv'), header=Fale, index=False)
pd.concat(val_y, val_X, axis=1).to_csv(os.path.join(data_dir, 'validation.csv'), header=Fale, index=False)
...
# ### (DONE) Uploading Training / Validation files to S3
# 
...
# ### (DONE) Creating the XGBoost model
# 
...
# In this case we have a binary label, thus the 'binary:logistic' objective.
...
# Transformer object for the trained model.
